...
function seitenausgabe($num_rows,$ds_pro_seite,$ds_count,$ajaxtype) {
...
function suchergebnis($ds_count,$num_rows,$fineselect) {
	...
	if ($fineselect) {
		...
		if (!$wk_inhalt) {
			$reuckgabe .=  "<form action=\"\"><table width=\"100%\" border=\"0\">\n";
			$reuckgabe .=  "  <tr bgcolor=\"#F7E7CE\">\n";
			$reuckgabe .=  "    <td colspan=\"2\"><table width=\"100%\" border=\"0\">\n";
			$reuckgabe .=  "    <tr><td colspan=\"2\"><b>Suchergebnis verfeinern</b></td>\n";
			$reuckgabe .=  "	  </table></td>\n";
			$reuckgabe .=  "  </tr>\n";
			$reuckgabe .=  "  <tr>\n";
			$reuckgabe .=  "    <td>\n";
	...
	$reuckgabe .=  "<TR><TD ALIGN=\"right\" COLSPAN=\"2\">".seitenausgabe($num_rows,$ds_pro_seite,$ds_count,$ajaxtype)."</TD></TR>\n";
	...
	if ($fineselect) {
		if ($_REQUEST["sortierung"] != "" || $_REQUEST["sortierung"] != "keine") {
			...
			$reuckgabe .=  "<a href=\"/home/firmen/einblick.phtml?suchwahl=volltext_new\">zur Eingabemaske</a>";
		}
	}else {
...
elseif ($_REQUEST["sortierung"] != "" && $_REQUEST["sortierung"] != "keine") {
	...
	if ($resultquery && mysql_numrows($resultquery) > 0) {
		while ($row = mysql_fetch_array($resultquery, MYSQL_NUM)) {
			$rueckgabe .= "<input type=\"checkbox\" name=\"".$sortierung.$row[0]."\" value=\"".$row[0]."\" class=\"checkbox_radio\">&nbsp;<a href=\"javascript:LinkSubmit('".$sortierung."','".$row[0]."')\">".$row[1]."</a>&nbsp;(".$row[2].")<br>";
		}
		$rueckgabe .= "<hr><input type=\"button\" value=\"Auswahl suchen\" onClick=\"CheckboxSubmit()\">";
	} else {
		$rueckgabe .= "Es wurden keine Kandidaten gefunden, die auf Ihre Sucheingabe passen, bitte versuchen Sie es erneut.";
	}
	$rueckgabe .= "<br><br><a href=\"/home/firmen/einblick.phtml?suchwahl=volltext_new\">zur Eingabemaske</a></form>";

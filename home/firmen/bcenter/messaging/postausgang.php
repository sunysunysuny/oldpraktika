...
$sql = '
			...
			WHERE
				...
				AND bewerberkontakte.firmenid != 0
...
?>
		<div class="bewerbungHeader">
			...
			<a href="/recruiting/nachrichtendetails/<?=$message['mitteilungsid'].'/'.$message['spalte'] ?>/" onclick="smallbox.loadUrl('', this.href); return false;"><?=htmlentities(stripslashes($message['betreff'])) ?></a>
			
		</div>
		<div class="bewerbungBody">
		<div class="actionButtons">
			<a alt="Ansehen" title="Ansehen"  href="/recruiting/nachrichtendetails/<?=$message['mitteilungsid'].'/'.$message['spalte'] ?>/" onclick="smallbox.loadUrl('', this.href); return false;">
...
<!--			<a href="/recruiting/weiterleitung_kurzmitteilung/<?=$message['mitteilungsid'].'/'.$message['spalte'] ?>/" alt="Weiterleiten" title="Weiterleiten"  onclick="return GB_showCenter('',this.href,400,700)">
			...
			<a href="#" alt="Löschen" title="Löschen"  onclick="deleteMessage(<?=$message['mitteilungsid'] ?>,'<?=md5($cryptbasis.$message['mitteilungsid'].$_SESSION["s_firmenid"].$cryptbasis) ?>'); return false;">
				<img src="/styles/images/icons/papierkorb.png" alt="Löschen" title="Löschen"  /></a>
</div>			
			...
			<br />
			<p style="float:right;">Mitteilung von: <?=(!empty($message["b_anrede"])?$message["b_anrede"]." ".$message["b_vname"]." ".$message["b_name"]:"Alle") ?></p>
			<p>

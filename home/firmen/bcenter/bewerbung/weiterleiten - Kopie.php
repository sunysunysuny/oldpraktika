...
if($_SERVER["HTTP_X_REQUESTED_WITH"] == "XMLHttpRequest" && $_POST["act"] == "send") {
	...
	$objBewerbung = new Praktika_Kcenter_Bewerbung($bid);
	if(!$objBewerbung->checkAuth()) {
		echo "Sie besitzen nicht die notwendigen Zugriffsrechte!";
		exit();
	}
	$result = $objBewerbung->weiterleitung($_POST["email"], $_POST["name"], Praktika_String::getIsoString($_POST["kommentar"]));
...
<p class="error" style='display:none;' id='weiterleitung-result'></p><br />
